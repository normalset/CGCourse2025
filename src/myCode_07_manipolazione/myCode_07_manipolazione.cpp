...
*   Ne segue che se cambiamo la proiezione questa funzione non calcola piu' il raggio giusto.
...
*/
void viewport_to_ray(GLFWwindow * window, double pX, double pY, glm::vec4& orig, glm::vec4& dir) {

	int h, w; 
	glfwGetWindowSize(window, &w, &h);

	glm::vec2 pos2;
	pos2.x = (float)(-1.0 + (pX / w) * (1.0 - (-1.0)));
	pos2.y = (float)(-0.8 + ((h - pY) / h) * (0.8 - (-0.8)));

	//pos2.x = (float)(-1.0 + (pX / 1000.0) * (1.0 - (-1.0)));
	//pos2.y = (float)(-0.8 + ((800.0 - pY) / 800.0) * (0.8 - (-0.8)));
...
*/
bool cursor_sphere_intersection(GLFWwindow* window , glm::vec3& int_point, double xpos, double ypos) {
	...
	*/
	viewport_to_ray(window, xpos, ypos, orig, dir);
...
static void cursor_position_callback(GLFWwindow* window, double xpos, double ypos)
{
	...
	if (cursor_sphere_intersection(window,p1, xpos, ypos)) {
...
void mouse_button_callback(GLFWwindow* window, int button, int action, int mods)
{
	if (button == GLFW_MOUSE_BUTTON_LEFT && action == GLFW_PRESS) {
		...
		*/
		if (cursor_sphere_intersection(window, int_point, xpos, ypos)) {
...
int main(void)
{
	...
	window = glfwCreateWindow(1200, 800, "code_07_manipulation", NULL, NULL);
	if (!window)
	{
		glfwTerminate();
		return -1;
