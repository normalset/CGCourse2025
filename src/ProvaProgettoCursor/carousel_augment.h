...
void inline prepareTerrain(race r, renderable& r_terrain) {
   ...
   // Generate triangle indices for the terrain grid
   const unsigned int Z = r.ter().size_pix[1];
   const unsigned int X = r.ter().size_pix[0];
   std::vector<GLuint> indices;
   for (unsigned int iz = 0; iz < Z - 1; ++iz) {
       for (unsigned int ix = 0; ix < X - 1; ++ix) {
           // First triangle
           indices.push_back(iz * X + ix);
           indices.push_back(iz * X + ix + 1);
           indices.push_back((iz + 1) * X + ix);
           
           // Second triangle
           indices.push_back(iz * X + ix + 1);
           indices.push_back((iz + 1) * X + ix + 1);
           indices.push_back((iz + 1) * X + ix);
       }
   }
   r_terrain.add_indices<GLuint>(&indices[0], static_cast<unsigned int>(indices.size()), GL_TRIANGLES);
